...
var optionDefaults = {
  ...
  /**
   * The name of the document array "friends" path
   */
  pathName: "_friends"
...
function plugin (options) {
  ...
  var updateFriendship = function (m1, m2, fship) {
    var query = {_id: m1._id};
    query[pathName] = {$elemMatch: {_id: m2._id}};
  ...
  var friendshipBetween = function (m1, m2) {
    var proj = {};
    proj[pathName] = {$elemMatch: {_id: m2._id}};
  ...
  return function friends (schema) {
     ...
     *
     * @param {Model} m1 the "friender" model doc doing the reqesting
     * @param {Model} m2 the "friendee" model doc being requested
    ...
    schema.statics.requestFriend = function (m1, m2, cb) {
      ...
      async.auto({
        ...
        steps.friender = ((hasfship)
          ? updateFriendship(m1, m2, fship, cb)
          : pushFriendship(m1, m2, fship, cb)).bind(Model)
    ...
    schema.statics.removeFriendship = function (m1, m2, cb) {
      ...
      var pull = function (m1, m2) {
        var update = {$pull: {}}
        update.$pull[pathName] = {_id: m2._id};
        return function (done) {
          collection.update({_id: m1._id}, update, done);
        }
      }
